...
$sw_wavelet_flag = "" ;
$WRFCHEM = 0 ;
#if ( WRF_PLANET == 1 ) # Flag to identify pWRF additions
$WRFPLANET = 0 ;
$WRFMARS = 0 ;
#endif
...
#   look for each known option
...
#if ( WRF_PLANET == 1 ) # Flag to identify pWRF additions
    $where_index = index ( $sw_compileflags , "-DWRF_PLANET" ) ;
    if ( $where_index eq -1 ) 
    {
      $WRFPLANET = 0 ;
    }
    ...
    $where_index = index ( $sw_compileflags , "-DWRF_MARS" ) ;
    if ( $where_index eq -1 ) 
    {
      $WRFMARS = 0 ;
    }
...
#if ( WRF_PLANET == 1 ) # Flag to identify pWRF additions
      @machopts = ( @machopts, "WRF_PLANET\t=\t$WRFPLANET \n" ) ;
      @machopts = ( @machopts, "WRF_MARS\t=\t$WRFMARS \n" ) ;
#endif
    }
  }
